...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
			...
			LDR R1, =SSI0_SR_R			;CHECK SSI0 STATUS
commSRprecheck     LDR R2, [R1]					
			AND R2, R2, #0x10
			CMP R2, #0						
			BNE commSRprecheck					;LOOP UNTIL SSI0 SR INDICATES NOT BUSY
			
			LDR R1, =DC					;SET UP COMMUNICATION FOR COMMAND WRITE	
			LDR R2, [R1]				
			AND R2, R2, #~0x40			;CLEAR PA6 FOR COMMAND WRITE	
			STR R2, [R1]
			
			LDR	R1, =SSI0_DR_R			;WRITE 8-BIT COMMAND TO SSI0 DR FOR TRANSMISSION
			STR R0, [R1]
			
			LDR R1, =SSI0_SR_R			;CHECK SSI0 STATUS
commSRpostcheck     LDR R2, [R1]					
			AND R2, R2, #0x10
			CMP R2, #0						
			BNE commSRpostcheck	
...
;4) Write the 8-bit data to SSI0_DR_R
			LDR R1, =SSI0_SR_R			;CHECK SSI0 STATUS
dataSRprecheck  LDR R2, [R1]					
			AND R2, R2, #0x02
			CMP R2, #0x02						
			BNE dataSRprecheck					;LOOP UNTIL SSI0 SR INDICATES NOT BUSY
			
			LDR R1, =GPIO_PORTA_DATA_R					;SET UP COMMUNICATION FOR DATA WRITE	
			LDR R2, [R1]				
			ORR R2, R2, #0x40			;SET PA6 HIGH FOR DATA WRITE	
			STR R2, [R1]
			
			LDR	R1, =SSI0_DR_R			;WRITE 8-BIT DATA TO SSI0 DR FOR TRANSMISSION
			STR R0, [R1]
			
;			LDR R1, =SSI0_SR_R			;CHECK SSI0 STATUS
;dataSRpostcheck     LDR R2, [R1]					
;			AND R2, R2, #0x10
;			CMP R2, #0						
;			BNE dataSRpostcheck	
    BX  LR                          ;   return to main program
