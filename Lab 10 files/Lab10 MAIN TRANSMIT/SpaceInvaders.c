...
uint32_t speed = 2000000;  //select speed settings for the LC Battle Dificulty
...
void Movedisc(int y){
	...
	Delay(1);
ST7735_DrawBitmap(discx,discy, bootdisc, 22, 9);
	}
}
struct Bike1 {
...
int MoveP1(int h, int v){
	...
	if((Player1[1].y)> 254 ){ //check y position
		Player1[3].life = 0;
		return 1;  //error, do nothing
		}
while(h !=0 || v !=0){
...
int MoveP2(int h, int v){ 
	...
	if((Player2[1].y)> 254 ){ //check y position
		Player2[3].life = 0;
		return 1;  //error, do nothing
		}
 while(h !=0 || v !=0){
...
ST7735_DrawBitmap(68,29, bootdisc, 22, 9);
